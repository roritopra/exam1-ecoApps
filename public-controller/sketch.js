...
function mouseReleased() {
    ...
    if (!buttonReleased(leftButton) && !buttonReleased(rightButton)) {
        ...
        socket.emit('movement-stopped', leftButton, rightButton);
...
function sendDirection(button) {
    if (dist(mouseX, mouseY, button.x, button.y) < 50) {
        socket.emit('move-controls', button)
        console.log('Its working👌', mouseX);
    }
};
